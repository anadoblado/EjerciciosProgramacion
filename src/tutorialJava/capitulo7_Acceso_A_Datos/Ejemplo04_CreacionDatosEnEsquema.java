...
public class Ejemplo04_CreacionDatosEnEsquema {
	...
	private static int getIdAzarEnTabla (Connection conn, String tabla) throws SQLException {
		...
		ResultSet rs;
		do {
			idAzar = ExamenObjetos2018.Utils.obtenerNumeroAleatorio(1, maxId);
					//Utils.obtenerNumeroAzar(1, maxId);
	...
	private static String getFechaAzar (int anioMin, int anioMax, SimpleDateFormat sdf) {
		Calendar calendar = new GregorianCalendar();// añadirlo con mi utils
//				(Utils.obtenerNumeroAzar(anioMin, anioMax) /*año*/, Utils.obtenerNumeroAzar(0, 11) /*mes*/, Utils.obtenerNumeroAzar(1, 31) /*día*/, 
//				Utils.obtenerNumeroAzar(0, 23) /*hora*/, Utils.obtenerNumeroAzar(0, 59) /*minuto*/, Utils.obtenerNumeroAzar(0, 59) /*segundo*/);
	...
	private static String getModeloCoche () {
		...
		for (int i = 0; i < 2; i++) {
			sb.append(abecedario.charAt(examenA.Utils.obtenerNumeroAleatorio(0, abecedario.length()-1)));
					//Utils.obtenerNumeroAzar(0, abecedario.length()-1)));
		}
		for (int i = 0; i < 2; i++) {
			sb.append(examenA.Utils.obtenerNumeroAleatorio(0, 9));
			//(Utils.obtenerNumeroAzar(0, 9));
		}
	...
	private static String getStringAlAzar (String strings[]) {
		return strings[examenA.Utils.obtenerNumeroAleatorio(0, strings.length - 1)];
		               //Utils.obtenerNumeroAzar(0, strings.length - 1)];
	}
	...
	private static float getPrecioAzar (int min, int max) {
		return (float) (examenA.Utils.obtenerNumeroAleatorio(min, max - 1) + (examenA.Utils.obtenerNumeroAleatorio(0, 99)/100.0));
				//Utils.obtenerNumeroAzar(min, max-1) + (Utils.obtenerNumeroAzar(0, 99) / 100.0));
	}
